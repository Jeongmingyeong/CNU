...
Python has experienced significant growth over the past decade and is now one
of the most widely used programming languages worldwide. Its popularity has
sharply increased since 2018 and it has consistently ranked as the most popular
language in multiple surveys since 2022~\cite{tiobe}.
This rise stems from Python’s versatility across domains such as web
development, data analytics, scientific computing, automation, and especially
artificial intelligence and machine learning.
The expansion of AI ecosystems—supported by deep learning frameworks (e.g.,
TensorFlow~\cite{tensorflow}, PyTorch~\cite{pytorch}) and natural language
processing libraries (e.g., Hugging Face Transformers~\cite{huggingface})—has
further accelerated its adoption among both industry and research communities.
...
code and it allows rapid prototyping.
...
runtime errors in Python stem from type mismatches~\cite{pyinder2024}.
...
or unexpected type usage can propagate subtle bugs across
modules~\cite{constypeerror}.

Consequently, type checking and type analysis have become critical for
improving the reliability and maintainability of Python programs.
...
verification while preserving the flexibility~\cite{pep484}.
Tools such as \texttt{mypy}~\cite{mypy}, \texttt{Pyre}~\cite{pyre}, and
\texttt{Pyright}~\cite{pyright} leverage these annotations to detect type
inconsistencies before execution, reducing the likelihood of runtime failures.
...
runtime and hybrid monitoring systems, and recently developed AI-powered
methods enabled by large language models.
