...
def pixel_bilinear_coordinate(src, pixel_coordinate):
    ...
    # pixel coordinate : 가 뭘까? -> 일단 형태는 float * float tuple
    ####################################################################################
    ...
    # bilinear interpolation (주석은 이전 실습 버전)
...
def non_maximum_supression_five_size(magnitude, angle, step = 0.5):
    ...
    for row in range(2, h - 2):
        for col in range(2, w - 2):
            ...
            if 0 <= degree and degree < 45:
                for i in np.arange(-2, 2.5, step):
                    if (i != 0) :
                        ...
                        pixel_coordinate.append(((row + rate), (col + i)))
                        # pixel_magnitude[row, col] = pixel_bilinear_coordinate(magnitude, pixel_coordinate)
                ...
                for i in np.arange(-2, 2.5, step):
                    if (i != 0):
                        ...
                        pixel_coordinate.append(((row + i), (col + rate)))
                        # pixel_magnitude[row, col] = pixel_bilinear_coordinate(magnitude, pixel_coordinate)
                        # if magnitude[row, col] == max(pixel_magnitude, magnitude[row, col]):
                        #     largest_magnitude[row, col] = magnitude[row, col]
            ...
            for i in pixel_coordinate: # i 는 튜플
                pixel_magnitude.append(pixel_bilinear_coordinate(magnitude, i))

            # 정상코드
            pixel_magnitude.append(magnitude[row, col])
            if magnitude[row, col] == max(pixel_magnitude):
                largest_magnitude[row, col] = magnitude[row, col]

            # 이게 나쁜 코드
            for i in pixel_magnitude:
                if magnitude[row, col] == max(i, magnitude[row, col]):
                    largest_magnitude[row, col] = magnitude[row, col]
...
def search_weak_edge(dst, edges, high_threshold_value, low_threshold_value, coordinate):
    ...
    # 1번 좌표
    if row > 0 and col > 0:
        # coordinates = Find_connected_dots_4neighbors(src, row - 1, col - 1, coordinates)
