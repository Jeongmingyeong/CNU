...
void eval(char *cmdline) 
{
	...
	if(!builtin_cmd(argv)) {
		if((pid = fork()) == 0) { /* Child Process인 경우, execve() 수행 */
...
int builtin_cmd(char **argv)
{
	...
	if(!strcmp(cmd, "quit")) { /* quit command */ 
		...
		listjobs(jobs, STDOUT_FILENO); /* listjobs's argument int output_fd : output file descriptor */
		return 1;
	}
