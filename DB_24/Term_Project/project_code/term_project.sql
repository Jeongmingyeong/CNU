...
('김밥', 5000), ('떡볶이', 6000), ('라볶이', 7000), ('치즈떡볶이', 7000),
('치즈라볶이', 8000), ('냉면', 8000), ('우동', 6000), ('김치찌개', 8000),
('순두부찌개', 8000), ('된장찌개', 8000), ('부대찌개', 10000), ('돈까스', 12000),
('과일빙수', 12000), ('팥빙수', 9000), ('소프트콘', 1200), ('공기밥', 1000),
('콜라', 2000), ('사이다', 2000), ('소주', 5000), ('맥주', 5000);
...
('김밥', '분식'), ('떡볶이', '분식'), ('라볶이', '분식'), ('치즈떡볶이', '분식'),
('치즈라볶이', '분식'), ('냉면', '분식'), ('냉면', '시즌메뉴(여름)'), ('우동', '분식'),
('김치찌개', '한식'), ('순두부찌개', '한식'), ('된장찌개', '한식'), ('부대찌개', '한식'),
('돈까스', '한식'), ('과일빙수', '디저트'), ('과일빙수', '시즌메뉴(여름)'),
('팥빙수', '디저트'), ('팥빙수', '시즌메뉴(여름)'), ('소프트콘', '디저트'),
('공기밥', '추가 및 음료'), ('콜라', '추가 및 음료'), ('사이다', '추가 및 음료'),
('소주', '추가 및 음료'), ('맥주', '추가 및 음료');
...
('C0', '관리자', '1111', '010-1111-1111'), ('C1', '정민경', '1234', '010-1234-5678'),
('C2', '홍길동', '0000', '010-9876-6789'), ('C3', '오백이', '5252', '010-5025-0250');

INSERT INTO Cart (id, orderDateTime, cno) VALUES
('1', CURRENT_TIMESTAMP, 'C3'), ('2', CURRENT_TIMESTAMP - INTERVAL 30 DAY, 'C3'),
('3', CURRENT_TIMESTAMP - INTERVAL 3 DAY, 'C1');
...
(2, '3', 1, (SELECT price FROM Food WHERE foodName = '과일빙수') * 1, '과일빙수');
...
SELECT 
    ...
    m.month AS 월, 
    IFNULL(SUM(CASE WHEN MONTH(c.orderDateTime) = m.month AND YEAR(c.orderDateTime) = 2024 THEN od.totalPrice ELSE 0 END), 0) AS 총판매가격
FROM 
    categories ct 
    CROSS JOIN months m
    LEFT JOIN Contain ctn ON ct.categoryName = ctn.categoryName
    LEFT JOIN OrderDetail od ON ctn.foodName = od.foodName
    LEFT JOIN Cart c ON od.id = c.id
GROUP BY ct.categoryName, m.month WITH ROLLUP
ORDER BY 월, 음식종류;

-- most population food --
WITH countofsale AS (
    SELECT 
        ct.categoryName, 
        od.foodName, 
